...
static int screen_x;
static int screen_y;
static char* video_mem = (char *)0x10000000; //(char *)VIDEO;

void scroll_row(){
	int  x,y;
	char terminal_num = *(uint8_t *)(video_mem + ((NUM_COLS*(NUM_ROWS-1) + NUM_COLS-1) << 1));
	for (y = 0; y < NUM_ROWS-1; y++)
	{
		for (x = 0; x < NUM_COLS; x++)
		{
			if (y == NUM_ROWS-2 && x == NUM_COLS-1)
			{
				//Keep Terminal Number
				if ((terminal_num == '1')||(terminal_num == '2')||(terminal_num == '3'))
				{
	...
	if ((terminal_num == '1')||(terminal_num == '2')||(terminal_num == '3'))
	{
		*(uint8_t *)(video_mem + ((NUM_COLS*(NUM_ROWS-1) + NUM_COLS-1) << 1)) = terminal_num;
...
/* write character at one previous location */
void putc_back(uint8_t c){
	screen_x--;
	screen_x %= NUM_COLS;
    screen_y = (screen_y + (screen_x / NUM_COLS)) % NUM_ROWS;
...
// write num to the bottom right of the screen
void write_terminal_number(uint8_t num){
	char num_char;
	if(num == 1){
		num_char = '1';
	}
    ...
    *(uint8_t *)(video_mem + ((NUM_COLS*24 + 79) << 1)) = num_char;
    *(uint8_t *)(video_mem + ((NUM_COLS*24 + 79) << 1) + 1) = ATTRIB;
}

/* start writing at next row */
void increment_row(){
	screen_y++;
	//screen_y%=NUM_ROWS;
	screen_x = 0;
...
/* start writing at previous row */
void decrement_row(){
	screen_y--;
	screen_x = NUM_COLS;
}

/* reset screen pointers to top left corner */
void reset_screen(){
	screen_x = 0;
	screen_y = 0;
}
